  ...
  try {
    ...
    try {
      await session.withTransaction(async () => {
        const entry = await ShopIn.findById(id).session(session);
        ...
        await ShopIn.findByIdAndDelete(id).session(session);
      });

      return NextResponse.json({ status: "ok", message: "Entry deleted and stock reversed" });
    } catch (err) {
